  ...
  const testFloatsWithCurrencyAndIgnoreDecimal = cloneDeep(testFloatsWithCurrency).map((row) => {
    ...
    switch (true) {
      case value >= 0 && value < 1:
        return [value, 'صفر دينار عراقي فقط لا غير'];
      ...
      case value >= 37 && value < 38:
        return [value, 'سبعة و ثلاثون دينار عراقي فقط لا غير'];
      case value >= 100 && value < 101:
        return [value, 'مائة دينار عراقي فقط لا غير'];
      case value >= 500 && value < 501:
        return [value, 'خمسمائة دينار عراقي فقط لا غير'];
      default:
        return row;
    }
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `مائة و ثلاثة و عشرون دينار عراقي فقط لا غير`],
    ['123', `مائة و ثلاثة و عشرون دينار عراقي فقط لا غير`],
    ['123.0', `مائة و ثلاثة و عشرون دينار عراقي و صفر فلس فقط لا غير`],
    ['123.000', `مائة و ثلاثة و عشرون دينار عراقي و صفر فلس فقط لا غير`],
    ['0.000', `صفر دينار عراقي و صفر فلس فقط لا غير`],
    ['-123.000', `سالب مائة و ثلاثة و عشرون دينار عراقي و صفر فلس فقط لا غير`],
    ['37.068', `سبعة و ثلاثون دينار عراقي و ثمانية و ستون فلس فقط لا غير`],
  ];
