...
import { ToWords as LocaleToWords } from '../src/locales/ar-SA.js';
...
const testBigIntsWithCurrency: [bigint, string][] = [
  [0n, 'صفر ريال فقط لا غير'],
  [1n, 'ريال واحد فقط لا غير'],
  [2n, 'ريالان فقط لا غير'],
  [100n, 'مائة ريال فقط لا غير'],
  [1000n, 'ألف ريال فقط لا غير'],
  [1000000n, 'مليون ريال فقط لا غير'],
];
...
describe('Test Zero Variants', () => {
  ...
  test('converts 0 with currency', () => {
    expect(toWords.convert(0, { currency: true })).toBe('صفر ريال فقط لا غير');
  });
...
describe('Test Currency', () => {
  test('1 Riyal', () => {
    expect(toWords.convert(1, { currency: true })).toBe('ريال واحد فقط لا غير');
  });

  test('2 Riyals', () => {
    expect(toWords.convert(2, { currency: true })).toBe('ريالان فقط لا غير');
  });

  test('100 Riyals', () => {
    expect(toWords.convert(100, { currency: true })).toBe('مائة ريال فقط لا غير');
  });
});
...
describe('Test All Currency Options Combinations', () => {
  ...
  test('currency only', () => {
    expect(toWords.convert(testValue, { currency: true })).toBe('مائة ريال و خمسون هللة فقط لا غير');
  });

  test('currency + doNotAddOnly', () => {
    expect(toWords.convert(testValue, { currency: true, doNotAddOnly: true })).toBe('مائة ريال و خمسون هللة');
  });

  test('currency + ignoreDecimal', () => {
    expect(toWords.convert(testValue, { currency: true, ignoreDecimal: true })).toBe('مائة ريال فقط لا غير');
  });

  test('currency + doNotAddOnly + ignoreDecimal', () => {
    expect(toWords.convert(testValue, { currency: true, doNotAddOnly: true, ignoreDecimal: true })).toBe('مائة ريال');
  });
});
