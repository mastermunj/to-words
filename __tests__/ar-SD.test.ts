  ...
  const testFloatsWithCurrencyAndIgnoreDecimal = cloneDeep(testFloatsWithCurrency).map((row) => {
    ...
    switch (true) {
      case value >= 0 && value < 1:
        return [value, 'صفر جنيه سوداني فقط لا غير'];
      ...
      case value >= 37 && value < 38:
        return [value, 'سبعة و ثلاثون جنيه سوداني فقط لا غير'];
      case value >= 100 && value < 101:
        return [value, 'مائة جنيه سوداني فقط لا غير'];
      case value >= 500 && value < 501:
        return [value, 'خمسمائة جنيه سوداني فقط لا غير'];
      default:
        return row;
    }
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `مائة و ثلاثة و عشرون جنيه سوداني فقط لا غير`],
    ['123', `مائة و ثلاثة و عشرون جنيه سوداني فقط لا غير`],
    ['123.0', `مائة و ثلاثة و عشرون جنيه سوداني و صفر قرش فقط لا غير`],
    ['123.00', `مائة و ثلاثة و عشرون جنيه سوداني و صفر قرش فقط لا غير`],
    ['0.00', `صفر جنيه سوداني و صفر قرش فقط لا غير`],
    ['-123.00', `سالب مائة و ثلاثة و عشرون جنيه سوداني و صفر قرش فقط لا غير`],
    ['37.68', `سبعة و ثلاثون جنيه سوداني و ثمانية و ستون قرش فقط لا غير`],
  ];
