...
import enBd from '../src/locales/en-BD.js';
import {
  ToWords as LocaleToWords,
  toWords as localeToWords,
  toOrdinal as localeToOrdinal,
  toCurrency as localeToCurrency,
} from '../src/locales/en-BD.js';

const localeCode = 'en-BD';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toWords.getLocaleClass()).toBe(enBd);
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `One Hundred Twenty Three Taka Only`],
    ['123', `One Hundred Twenty Three Taka Only`],
    ['123.0', `One Hundred Twenty Three Taka And Zero Poysha Only`],
    ['123.00', `One Hundred Twenty Three Taka And Zero Poysha Only`],
    ['0.00', `Zero Taka And Zero Poysha Only`],
    ['-123.00', `Minus One Hundred Twenty Three Taka And Zero Poysha Only`],
    ['37.68', `Thirty Seven Taka And Sixty Eight Poysha Only`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, `Zero Taka Only`],
  [0.04, `Zero Taka And Four Poysha Only`],
  [0.0468, `Zero Taka And Five Poysha Only`],
  [0.4, `Zero Taka And Forty Poysha Only`],
  [0.63, `Zero Taka And Sixty Three Poysha Only`],
  [0.973, `Zero Taka And Ninety Seven Poysha Only`],
  [0.999, `One Taka Only`],
  [37.06, `Thirty Seven Taka And Six Poysha Only`],
  [37.068, `Thirty Seven Taka And Seven Poysha Only`],
  [37.68, `Thirty Seven Taka And Sixty Eight Poysha Only`],
  [37.683, `Thirty Seven Taka And Sixty Eight Poysha Only`],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Zero Taka And `, '');
    }
  });
...
describe('Test Floats with options = { currency: true, ignoreDecimal: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreDecimal.map((row) => {
    if (row[0] === 0.999) {
      row[1] = `Zero Taka Only`;
    } else {
      row[1] = (row[1] as string).replace(new RegExp(` And [\\w ]+ Poysha`), '');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('convert 0 with currency => Zero Taka Only', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Zero Taka Only');
  });
});
...
const testFractionStyleEnBD: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEnBD)('convert %d => %s', (input, expected) => {
  ...
  test('falls back to digit-by-digit for unmapped decimal length (7 places)', () => {
    expect(toWords.convert(1.1234567, { decimalStyle: 'fraction' })).toBe(
      'One Point Twelve Lakh Thirty Four Thousand Five Hundred Sixty Seven',
    );
    expect(toWords.convert(1.1234567)).toBe('One Point Twelve Lakh Thirty Four Thousand Five Hundred Sixty Seven');
  });
