...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `One Hundred Twenty Three Euros Only`],
    ['123', `One Hundred Twenty Three Euros Only`],
    ['123.0', `One Hundred Twenty Three Euros And Zero Cents Only`],
    ['123.00', `One Hundred Twenty Three Euros And Zero Cents Only`],
    ['0.00', `Zero Euros And Zero Cents Only`],
    ['-123.00', `Minus One Hundred Twenty Three Euros And Zero Cents Only`],
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, `Zero Euros Only`],
  [0.01, `Zero Euros And One Cent Only`],
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Zero Euros And `, '');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('convert 0 with currency => Zero Euros Only', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Zero Euros Only');
  });
});
...
const testFractionStyleEnIE: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEnIE)('convert %d => %s', (input, expected) => {
