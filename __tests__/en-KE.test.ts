...
import enKe from '../src/locales/en-KE.js';
import {
  ToWords as LocaleToWords,
  toWords as localeToWords,
  toOrdinal as localeToOrdinal,
  toCurrency as localeToCurrency,
} from '../src/locales/en-KE.js';

const localeCode = 'en-KE';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toWords.getLocaleClass()).toBe(enKe);
  });
...
describe('Test Negative Integers with options = { currency: true }', () => {
  ...
  testNegativeIntegersWithCurrency.map((row, i) => {
    ...
    row[0] = -row[0];
    row[1] = `Minus ${row[1]} Shillings Only`;
  });
...
describe('Test Integers with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testIntegersWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    row[1] = i === 0 ? '' : `${row[1]} Shillings Only`;
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `One Hundred Twenty Three Shillings Only`],
    ['123', `One Hundred Twenty Three Shillings Only`],
    ['123.0', `One Hundred Twenty Three Shillings And Zero Cents Only`],
    ['123.00', `One Hundred Twenty Three Shillings And Zero Cents Only`],
    ['0.00', `Zero Shillings And Zero Cents Only`],
    ['-123.00', `Minus One Hundred Twenty Three Shillings And Zero Cents Only`],
    ['37.68', `Thirty Seven Shillings And Sixty Eight Cents Only`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, `Zero Shillings Only`],
  [0.01, `Zero Shillings And One Cent Only`],
  [0.04, `Zero Shillings And Four Cents Only`],
  [0.0468, `Zero Shillings And Five Cents Only`],
  [0.4, `Zero Shillings And Forty Cents Only`],
  [0.63, `Zero Shillings And Sixty Three Cents Only`],
  [0.973, `Zero Shillings And Ninety Seven Cents Only`],
  [0.999, `One Shilling Only`],
  [37.06, `Thirty Seven Shillings And Six Cents Only`],
  [37.068, `Thirty Seven Shillings And Seven Cents Only`],
  [37.68, `Thirty Seven Shillings And Sixty Eight Cents Only`],
  [37.683, `Thirty Seven Shillings And Sixty Eight Cents Only`],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Zero Shillings And `, '');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('convert 0 with currency => Zero Shillings Only', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Zero Shillings Only');
  });
});
...
const testFractionStyleEnKE: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEnKE)('convert %d => %s', (input, expected) => {
