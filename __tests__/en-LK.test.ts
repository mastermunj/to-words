...
import enUs from '../src/locales/en-LK.js';
import {
  ToWords as LocaleToWords,
  toWords as localeToWords,
  toOrdinal as localeToOrdinal,
  toCurrency as localeToCurrency,
} from '../src/locales/en-LK.js';

const localeCode = 'en-LK';
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `One Hundred Twenty Three Rupees Only`],
    ['123', `One Hundred Twenty Three Rupees Only`],
    ['123.0', `One Hundred Twenty Three Rupees And Zero Cents Only`],
    ['123.00', `One Hundred Twenty Three Rupees And Zero Cents Only`],
    ['0.00', `Zero Rupees And Zero Cents Only`],
    ['-123.00', `Minus One Hundred Twenty Three Rupees And Zero Cents Only`],
...
// ============================================================
// COMPREHENSIVE TEST ADDITIONS FOR en-LK
// ============================================================
...
const testNegativeFloatsWithCurrency: [number, string][] = [
  [-0.5, 'Minus Zero Rupees And Fifty Cents Only'],
  [-0.99, 'Minus Zero Rupees And Ninety Nine Cents Only'],
  [-1.5, 'Minus One Rupee And Fifty Cents Only'],
  [-1.01, 'Minus One Rupee And One Cent Only'],
  [-100.5, 'Minus One Hundred Rupees And Fifty Cents Only'],
  [-1000.99, 'Minus One Thousand Rupees And Ninety Nine Cents Only'],
];
...
const testBigIntsWithCurrency: [bigint, string][] = [
  ...
  [1000000n, 'One Million Rupees Only'],
  [1000000000n, 'One Billion Rupees Only'],
];
...
const testStringInputsWithCurrency: [string, string][] = [
  ...
  ['100.50', 'One Hundred Rupees And Fifty Cents Only'],
...
describe('Test Currency Singular/Plural', () => {
  ...
  test('0.01 Cent (singular)', () => {
    expect(toWords.convert(0.01, { currency: true })).toBe('Zero Rupees And One Cent Only');
  });

  test('0.02 Cents (plural)', () => {
    expect(toWords.convert(0.02, { currency: true })).toBe('Zero Rupees And Two Cents Only');
  });

  test('1.01 (singular + singular)', () => {
    expect(toWords.convert(1.01, { currency: true })).toBe('One Rupee And One Cent Only');
  });

  test('2.02 (plural + plural)', () => {
    expect(toWords.convert(2.02, { currency: true })).toBe('Two Rupees And Two Cents Only');
  });
});
...
describe('Test All Currency Options Combinations', () => {
  ...
  test('currency only', () => {
    expect(toWords.convert(testValue, { currency: true })).toBe('One Hundred Rupees And Fifty Cents Only');
  });

  test('currency + doNotAddOnly', () => {
    expect(toWords.convert(testValue, { currency: true, doNotAddOnly: true })).toBe(
      'One Hundred Rupees And Fifty Cents',
    );
  });
...
// Invalid Input Tests
describe('Test Invalid Inputs for en-LK', () => {
  test('throws for NaN', () => {
...
const testFractionStyleEnLK: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEnLK)('convert %d => %s', (input, expected) => {
