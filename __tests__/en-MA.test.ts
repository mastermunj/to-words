...
describe('Test Integers with options = { currency: true }', () => {
  ...
  testIntegersWithCurrency.map((row) => {
    row[1] = row[0] === 1 ? `One Dirham Only` : `${row[1]} Dirhams Only`;
  });
...
describe('Test Integers with options = { currency: true, doNotAddOnly: true }', () => {
  const testIntegersWithCurrency = cloneDeep(testIntegers);
  testIntegersWithCurrency.map((row) => {
    row[1] = row[0] === 1 ? `One Dirham` : `${row[1]} Dirhams`;
  });
...
describe('Test Negative Integers with options = { currency: true }', () => {
  ...
  testNegativeIntegersWithCurrency.map((row, i) => {
    ...
    row[0] = -row[0];
    const currencyPart = row[0] === -1 ? 'Dirham' : 'Dirhams';
    row[1] = `Minus ${row[1]} ${currencyPart} Only`;
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `One Hundred Twenty Three Dirhams Only`],
    ['123', `One Hundred Twenty Three Dirhams Only`],
    ['123.0', `One Hundred Twenty Three Dirhams And Zero Centimes Only`],
    ['123.00', `One Hundred Twenty Three Dirhams And Zero Centimes Only`],
    ['0.00', `Zero Dirhams And Zero Centimes Only`],
    ['-123.00', `Minus One Hundred Twenty Three Dirhams And Zero Centimes Only`],
    ['37.68', `Thirty Seven Dirhams And Sixty Eight Centimes Only`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  ...
  [0.01, `Zero Dirhams And One Centime Only`],
  [0.04, `Zero Dirhams And Four Centimes Only`],
  [0.0468, `Zero Dirhams And Five Centimes Only`],
  [0.4, `Zero Dirhams And Forty Centimes Only`],
  [0.63, `Zero Dirhams And Sixty Three Centimes Only`],
  [0.973, `Zero Dirhams And Ninety Seven Centimes Only`],
  [0.999, `One Dirham Only`],
  [1, `One Dirham Only`],
  [37.06, `Thirty Seven Dirhams And Six Centimes Only`],
  [37.068, `Thirty Seven Dirhams And Seven Centimes Only`],
  [37.68, `Thirty Seven Dirhams And Sixty Eight Centimes Only`],
  [37.683, `Thirty Seven Dirhams And Sixty Eight Centimes Only`],
...
describe('Test Floats with options = { currency: true, ignoreDecimal: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreDecimal.map((row) => {
    if (row[0] === 0.999) {
      ...
      row[1] = `One Dirham Only`;
    } else {
      row[1] = (row[1] as string).replace(new RegExp(` And [\\w ]+ (Centimes|Centime)`), '');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('convert 0 with currency => Zero Dirhams Only', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Zero Dirhams Only');
  });
});
...
const testFractionStyleEnMA: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEnMA)('convert %d => %s', (input, expected) => {
