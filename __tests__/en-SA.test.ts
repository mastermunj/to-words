...
import enSa from '../src/locales/en-SA.js';
import {
  ToWords as LocaleToWords,
  toWords as localeToWords,
  toOrdinal as localeToOrdinal,
  toCurrency as localeToCurrency,
} from '../src/locales/en-SA.js';

const localeCode = 'en-SA';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toWords.getLocaleClass()).toBe(enSa);
  });
...
describe('Test Integers with options = { currency: true }', () => {
  ...
  testIntegersWithCurrency.map((row) => {
    row[1] = row[0] === 1 ? `One Riyal Only` : `${row[1]} Riyals Only`;
  });
...
describe('Test Integers with options = { currency: true, doNotAddOnly: true }', () => {
  const testIntegersWithCurrency = cloneDeep(testIntegers);
  testIntegersWithCurrency.map((row) => {
    row[1] = row[0] === 1 ? `One Riyal` : `${row[1]} Riyals`;
  });
...
describe('Test Negative Integers with options = { currency: true }', () => {
  ...
  testNegativeIntegersWithCurrency.map((row, i) => {
    if (i === 0) {
      row[1] = `${row[1]} Riyals Only`;
      return;
    }
    row[0] = -row[0];
    const currencyPart = row[0] === -1 ? 'Riyal' : 'Riyals';
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `One Hundred Twenty Three Riyals Only`],
    ['123', `One Hundred Twenty Three Riyals Only`],
    ['123.0', `One Hundred Twenty Three Riyals And Zero Halalas Only`],
    ['123.00', `One Hundred Twenty Three Riyals And Zero Halalas Only`],
    ['0.00', `Zero Riyals And Zero Halalas Only`],
    ['-123.00', `Minus One Hundred Twenty Three Riyals And Zero Halalas Only`],
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Zero Riyals And `, '');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('zero with currency', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Zero Riyals Only');
  });
});
...
const testFractionStyleEnSA: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEnSA)('convert %d => %s', (input, expected) => {
