...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toWords.getLocaleClass()).toBe(enZa);
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `One Hundred Twenty Three Rand Only`],
    ['123', `One Hundred Twenty Three Rand Only`],
    ['123.0', `One Hundred Twenty Three Rand And Zero Cents Only`],
    ['123.00', `One Hundred Twenty Three Rand And Zero Cents Only`],
    ['0.00', `Zero Rand And Zero Cents Only`],
    ['-123.00', `Minus One Hundred Twenty Three Rand And Zero Cents Only`],
    ['37.68', `Thirty Seven Rand And Sixty Eight Cents Only`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, `Zero Rand Only`],
  [0.01, `Zero Rand And One Cent Only`],
  [0.04, `Zero Rand And Four Cents Only`],
  [0.0468, `Zero Rand And Five Cents Only`],
  [0.4, `Zero Rand And Forty Cents Only`],
  [0.63, `Zero Rand And Sixty Three Cents Only`],
  [0.973, `Zero Rand And Ninety Seven Cents Only`],
  [0.999, `One Rand Only`],
  [37.06, `Thirty Seven Rand And Six Cents Only`],
  [37.068, `Thirty Seven Rand And Seven Cents Only`],
  [37.68, `Thirty Seven Rand And Sixty Eight Cents Only`],
  [37.683, `Thirty Seven Rand And Sixty Eight Cents Only`],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Zero Rand And `, '');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('convert 0 with currency => Zero Rand Only', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Zero Rand Only');
  });
});
...
describe('Test Invalid Inputs', () => {
  test.concurrent.each(invalidInputTests)('convert %s => throws error', (input, errorMessage) => {
    expect(() => toWords.convert(input as number)).toThrow(errorMessage);
  });
});
...
const testFractionStyleEnZA: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEnZA)('convert %d => %s', (input, expected) => {
