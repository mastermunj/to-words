...
import enZw from '../src/locales/en-ZW.js';
import {
  ToWords as LocaleToWords,
  toWords as localeToWords,
  toOrdinal as localeToOrdinal,
  toCurrency as localeToCurrency,
} from '../src/locales/en-ZW.js';

const localeCode = 'en-ZW';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toWords.getLocaleClass()).toBe(enZw);
  });
...
describe('Test Integers with options = { currency: true }', () => {
  ...
  testIntegersWithCurrency.map((row) => {
    row[1] = `${row[1]} Zimbabwe Gold Only`;
  });
...
describe('Test Negative Integers with options = { currency: true }', () => {
  ...
  testNegativeIntegersWithCurrency.map((row, i) => {
    ...
    row[0] = -row[0];
    row[1] = `Minus ${row[1]} Zimbabwe Gold Only`;
  });
...
describe('Test Integers with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testIntegersWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    row[1] = i === 0 ? '' : `${row[1]} Zimbabwe Gold Only`;
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `One Hundred Twenty Three Zimbabwe Gold Only`],
    ['123', `One Hundred Twenty Three Zimbabwe Gold Only`],
    ['123.0', `One Hundred Twenty Three Zimbabwe Gold And Zero Cents Only`],
    ['123.00', `One Hundred Twenty Three Zimbabwe Gold And Zero Cents Only`],
    ['0.00', `Zero Zimbabwe Gold And Zero Cents Only`],
    ['-123.00', `Minus One Hundred Twenty Three Zimbabwe Gold And Zero Cents Only`],
    ['37.68', `Thirty Seven Zimbabwe Gold And Sixty Eight Cents Only`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, `Zero Zimbabwe Gold Only`],
  [0.01, `Zero Zimbabwe Gold And One Cent Only`],
  [0.04, `Zero Zimbabwe Gold And Four Cents Only`],
  [0.0468, `Zero Zimbabwe Gold And Five Cents Only`],
  [0.4, `Zero Zimbabwe Gold And Forty Cents Only`],
  [0.63, `Zero Zimbabwe Gold And Sixty Three Cents Only`],
  [0.973, `Zero Zimbabwe Gold And Ninety Seven Cents Only`],
  [0.999, `One Zimbabwe Gold Only`],
  [37.06, `Thirty Seven Zimbabwe Gold And Six Cents Only`],
  [37.068, `Thirty Seven Zimbabwe Gold And Seven Cents Only`],
  [37.68, `Thirty Seven Zimbabwe Gold And Sixty Eight Cents Only`],
  [37.683, `Thirty Seven Zimbabwe Gold And Sixty Eight Cents Only`],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Zero Zimbabwe Gold And `, '');
    }
  });
...
describe('Test Floats with options = { currency: true, ignoreDecimal: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreDecimal.map((row) => {
    if (row[0] === 0.999) {
      row[1] = `Zero Zimbabwe Gold Only`;
    } else {
      row[1] = (row[1] as string).replace(new RegExp(` And [\\w ]+ (Cents|Cent)`), '');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('convert 0 with currency => Zero Zimbabwe Gold Only', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Zero Zimbabwe Gold Only');
  });
});
...
const testFractionStyleEnZW: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEnZW)('convert %d => %s', (input, expected) => {
