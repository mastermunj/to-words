...
describe('Test Negative Integers with options = { currency: true }', () => {
  ...
  testNegativeIntegersWithCurrency.map((row, i) => {
    ...
    row[0] = -row[0];
    row[1] = `Menos ${row[1]} Pesos Argentinos`;
  });
...
describe('Test Integers with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testIntegersWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    row[1] = i === 0 ? '' : `${row[1]} Pesos Argentinos`;
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `Ciento Veintitrés Pesos Argentinos`],
    ['123', `Ciento Veintitrés Pesos Argentinos`],
    ['123.0', `Ciento Veintitrés Pesos Argentinos Y Cero Centavos`],
    ['123.00', `Ciento Veintitrés Pesos Argentinos Y Cero Centavos`],
    ['0.00', `Cero Pesos Argentinos Y Cero Centavos`],
    ['-123.00', `Menos Ciento Veintitrés Pesos Argentinos Y Cero Centavos`],
    ['37.68', `Treinta Y Siete Pesos Argentinos Y Sesenta Y Ocho Centavos`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, `Cero Pesos Argentinos`],
  [0.04, `Cero Pesos Argentinos Y Cuatro Centavos`],
  [0.0468, `Cero Pesos Argentinos Y Cinco Centavos`],
  [0.4, `Cero Pesos Argentinos Y Cuarenta Centavos`],
  [0.63, `Cero Pesos Argentinos Y Sesenta Y Tres Centavos`],
  [0.973, `Cero Pesos Argentinos Y Noventa Y Siete Centavos`],
  [0.999, `Un Peso Argentino`],
  [37.06, `Treinta Y Siete Pesos Argentinos Y Seis Centavos`],
  [37.068, `Treinta Y Siete Pesos Argentinos Y Siete Centavos`],
  [37.68, `Treinta Y Siete Pesos Argentinos Y Sesenta Y Ocho Centavos`],
  [37.683, `Treinta Y Siete Pesos Argentinos Y Sesenta Y Ocho Centavos`],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Cero Pesos Argentinos Y `, '');
    }
  });
...
describe('Test Floats with options = { currency: true, ignoreDecimal: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreDecimal.map((row) => {
    if (row[0] === 0.999) {
      row[1] = `Cero Pesos Argentinos`;
    } else {
      row[1] = (row[1] as string).replace(new RegExp(` Pesos Argentinos Y [\\w ]+ Centavos`), ' Pesos Argentinos');
    }
  });
...
const testFractionStyleEsAR: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEsAR)('convert %d => %s', (input, expected) => {
