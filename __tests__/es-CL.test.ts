...
describe('Test Negative Integers with options = { currency: true }', () => {
  ...
  testNegativeIntegersWithCurrency.map((row, i) => {
    ...
    row[0] = -row[0];
    row[1] = `Menos ${row[1]} Pesos Chilenos`;
  });
...
describe('Test Integers with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testIntegersWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    row[1] = i === 0 ? '' : `${row[1]} Pesos Chilenos`;
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `Ciento Veintitrés Pesos Chilenos`],
    ['123', `Ciento Veintitrés Pesos Chilenos`],
    ['123.0', `Ciento Veintitrés Pesos Chilenos Y Cero Centavos`],
    ['123.00', `Ciento Veintitrés Pesos Chilenos Y Cero Centavos`],
    ['0.00', `Cero Pesos Chilenos Y Cero Centavos`],
    ['-123.00', `Menos Ciento Veintitrés Pesos Chilenos Y Cero Centavos`],
    ['37.68', `Treinta Y Siete Pesos Chilenos Y Sesenta Y Ocho Centavos`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, `Cero Pesos Chilenos`],
  [0.04, `Cero Pesos Chilenos Y Cuatro Centavos`],
  [0.0468, `Cero Pesos Chilenos Y Cinco Centavos`],
  [0.4, `Cero Pesos Chilenos Y Cuarenta Centavos`],
  [0.63, `Cero Pesos Chilenos Y Sesenta Y Tres Centavos`],
  [0.973, `Cero Pesos Chilenos Y Noventa Y Siete Centavos`],
  [0.999, `Un Peso Chileno`],
  [37.06, `Treinta Y Siete Pesos Chilenos Y Seis Centavos`],
  [37.068, `Treinta Y Siete Pesos Chilenos Y Siete Centavos`],
  [37.68, `Treinta Y Siete Pesos Chilenos Y Sesenta Y Ocho Centavos`],
  [37.683, `Treinta Y Siete Pesos Chilenos Y Sesenta Y Ocho Centavos`],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Cero Pesos Chilenos Y `, '');
    }
  });
...
describe('Test Floats with options = { currency: true, ignoreDecimal: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreDecimal.map((row) => {
    if (row[0] === 0.999) {
      row[1] = `Cero Pesos Chilenos`;
    } else {
      row[1] = (row[1] as string).replace(new RegExp(` Pesos Chilenos Y [\\w ]+ Centavos`), ' Pesos Chilenos');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('convert 0 with currency => Cero Pesos Chilenos', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Cero Pesos Chilenos');
  });
});
...
const testFractionStyleEsCL: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEsCL)('convert %d => %s', (input, expected) => {
