...
import esUS from '../src/locales/es-US.js';
import {
  ToWords as LocaleToWords,
  toWords as localeToWords,
  toOrdinal as localeToOrdinal,
  toCurrency as localeToCurrency,
} from '../src/locales/es-US.js';

const localeCode = 'es-US';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toWords.getLocaleClass()).toBe(esUS);
  });
...
describe('Test Integers with options = { currency: true }', () => {
  ...
  testIntegersWithCurrency.map((row) => {
    row[1] = `${row[1]} Dólares Estadounidenses`;
  });
...
describe('Test Negative Integers with options = { currency: true }', () => {
  ...
  testNegativeIntegersWithCurrency.map((row, i) => {
    ...
    row[0] = -row[0];
    row[1] = `Menos ${row[1]} Dólares Estadounidenses`;
  });
...
describe('Test Integers with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testIntegersWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    row[1] = i === 0 ? '' : `${row[1]} Dólares Estadounidenses`;
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `Ciento Veintitrés Dólares Estadounidenses`],
    ['123', `Ciento Veintitrés Dólares Estadounidenses`],
    ['123.0', `Ciento Veintitrés Dólares Estadounidenses Y Cero Centavos`],
    ['123.00', `Ciento Veintitrés Dólares Estadounidenses Y Cero Centavos`],
    ['0.00', `Cero Dólares Estadounidenses Y Cero Centavos`],
    ['-123.00', `Menos Ciento Veintitrés Dólares Estadounidenses Y Cero Centavos`],
    ['37.68', `Treinta Y Siete Dólares Estadounidenses Y Sesenta Y Ocho Centavos`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, `Cero Dólares Estadounidenses`],
  [0.04, `Cero Dólares Estadounidenses Y Cuatro Centavos`],
  [0.0468, `Cero Dólares Estadounidenses Y Cinco Centavos`],
  [0.4, `Cero Dólares Estadounidenses Y Cuarenta Centavos`],
  [0.63, `Cero Dólares Estadounidenses Y Sesenta Y Tres Centavos`],
  [0.973, `Cero Dólares Estadounidenses Y Noventa Y Siete Centavos`],
  [0.999, `Un Dólar Estadounidense`],
  [37.06, `Treinta Y Siete Dólares Estadounidenses Y Seis Centavos`],
  [37.068, `Treinta Y Siete Dólares Estadounidenses Y Siete Centavos`],
  [37.68, `Treinta Y Siete Dólares Estadounidenses Y Sesenta Y Ocho Centavos`],
  [37.683, `Treinta Y Siete Dólares Estadounidenses Y Sesenta Y Ocho Centavos`],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Cero Dólares Estadounidenses Y `, '');
    }
  });
...
describe('Test Floats with options = { currency: true, ignoreDecimal: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreDecimal.map((row) => {
    if (row[0] === 0.999) {
      row[1] = `Cero Dólares Estadounidenses`;
    } else {
      row[1] = (row[1] as string).replace(new RegExp(`Dólares Estadounidenses.*`), 'Dólares Estadounidenses');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('convert 0 with currency => Cero Dólares Estadounidenses', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Cero Dólares Estadounidenses');
  });
});
...
const testFractionStyleEsUS: [number, string][] = [
  [1.1, 'Uno Y Uno Décimo'],
  [2.5, 'Dos Y Cinco Décimos'],
  [1.01, 'Uno Y Uno Centésimo'],
  [1.45, 'Uno Y Cuarenta Y Cinco Centésimos'],
  [0.05, 'Cero Y Cinco Centésimos'],
  [1.001, 'Uno Y Uno Milésimo'],
  [1.005, 'Uno Y Cinco Milésimos'],
  [1.0001, 'Uno Y Uno Diezmilésimo'],
  [1.0005, 'Uno Y Cinco Diezmilésimos'],
  [1.00001, 'Uno Y Uno Cienmilésimo'],
  [1.00005, 'Uno Y Cinco Cienmilésimos'],
  [1.000001, 'Uno Y Uno Millonésimo'],
  [1.000005, 'Uno Y Cinco Millonésimos'],
  [123.45, 'Ciento Veintitrés Y Cuarenta Y Cinco Centésimos'],
];

describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleEsUS)('convert %d => %s', (input, expected) => {
  ...
  test('falls back to digit-by-digit for unmapped decimal length (7 places)', () => {
    expect(toWords.convert(1.1234567, { decimalStyle: 'fraction' })).toBe(
      'Uno Punto Un Millon Doscientos Treinta Y Cuatro Mil Quinientos Sesenta Y Siete',
    );
  ...
  test('digit-by-digit style works without decimalStyle option', () => {
    expect(toWords.convert(1.5)).toBe('Uno Punto Cinco');
    expect(toWords.convert(0.05)).toBe('Cero Punto Cero Cinco');
  });
});
