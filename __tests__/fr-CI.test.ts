...
import frCi from '../src/locales/fr-CI.js';
import {
  ToWords as LocaleToWords,
  toWords as localeToWords,
  toOrdinal as localeToOrdinal,
  toCurrency as localeToCurrency,
} from '../src/locales/fr-CI.js';

const localeCode = 'fr-CI';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toWords.getLocaleClass()).toBe(frCi);
  });
...
describe('Test Integers with options = { currency: true }', () => {
  ...
  testIntegersWithCurrency.map((row) => {
    row[1] = row[0] === 1 ? `Un Franc CFA` : `${row[1]} Francs CFA`;
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `Cent Vingt-Trois Francs CFA`],
    ['123', `Cent Vingt-Trois Francs CFA`],
    ['123.0', `Cent Vingt-Trois Francs CFA Et Zéro Centimes`],
    ['123.00', `Cent Vingt-Trois Francs CFA Et Zéro Centimes`],
    ['0.00', `Zéro Francs CFA Et Zéro Centimes`],
    ['-123.00', `Moins Cent Vingt-Trois Francs CFA Et Zéro Centimes`],
    ['37.68', `Trente-Sept Francs CFA Et Soixante-Huit Centimes`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, 'Zéro Francs CFA'],
  [0.04, 'Zéro Francs CFA Et Quatre Centimes'],
  [0.4, 'Zéro Francs CFA Et Quarante Centimes'],
  [0.63, 'Zéro Francs CFA Et Soixante-Trois Centimes'],
  [0.973, 'Zéro Francs CFA Et Quatre-Vingt-Dix-Sept Centimes'],
  [0.999, 'Un Franc CFA'],
  [1, 'Un Franc CFA'],
  [37.06, 'Trente-Sept Francs CFA Et Six Centimes'],
  [37.68, 'Trente-Sept Francs CFA Et Soixante-Huit Centimes'],
  [37.683, 'Trente-Sept Francs CFA Et Soixante-Huit Centimes'],
  [100, 'Cent Francs CFA'],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace('Zéro Francs CFA Et ', '');
    }
  });
  ...
  const testFloatsWithCurrencyAndIgnoreDecimal = cloneDeep(testFloatsWithCurrency).map((row) => {
    ...
    switch (true) {
      case value >= 0 && value < 1:
        return [value, 'Zéro Francs CFA'];
      case value >= 1 && value < 2:
        return [value, 'Un Franc CFA'];
      case value >= 37 && value < 38:
        return [value, 'Trente-Sept Francs CFA'];
      case value >= 100 && value < 101:
        return [value, 'Cent Francs CFA'];
      default:
        return row;
    }
  });
...
describe('Test Locale functional exports', () => {
  ...
  test('localeToCurrency works', () => {
    expect(localeToCurrency(1)).toBe('Un Franc CFA');
  });
});
