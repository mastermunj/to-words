...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `એક સો તેવીસ રૂપિયા`],
    ['123', `એક સો તેવીસ રૂપિયા`],
    ['123.0', `એક સો તેવીસ રૂપિયા અને શૂન્ય પૈસા`],
    ['123.00', `એક સો તેવીસ રૂપિયા અને શૂન્ય પૈસા`],
    ['0.00', `શૂન્ય રૂપિયા અને શૂન્ય પૈસા`],
    ['-123.00', `ઋણ એક સો તેવીસ રૂપિયા અને શૂન્ય પૈસા`],
...
describe('Test Zero Variants', () => {
  ...
  test('zero with currency', () => {
    expect(toWords.convert(0, { currency: true })).toBe('શૂન્ય રૂપિયા');
  });
});
...
describe('Test Invalid Inputs', () => {
  test.concurrent.each([
    ...
    ['', /Invalid Number/],
    ['abc', /Invalid Number/],
  ] as [number | string, RegExp][])('convert %s should throw', (input, expectedError) => {
