...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `Seratus Dua Puluh Tiga Rupiah Saja`],
    ['123', `Seratus Dua Puluh Tiga Rupiah Saja`],
    ['123.0', `Seratus Dua Puluh Tiga Rupiah Dan Nol Sen Saja`],
    ['123.00', `Seratus Dua Puluh Tiga Rupiah Dan Nol Sen Saja`],
    ['0.00', `Nol Rupiah Dan Nol Sen Saja`],
    ['-123.00', `Minus Seratus Dua Puluh Tiga Rupiah Dan Nol Sen Saja`],
...
const testFractionStyleIdID: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleIdID)('convert %d => %s', (input, expected) => {
  ...
  test('falls back to digit-by-digit for unmapped decimal length (7 places)', () => {
    expect(toWords.convert(1.1234567, { decimalStyle: 'fraction' })).toBe(
      'Satu Koma Satu Juta Dua Ratus Tiga Puluh Empat Ribu Lima Ratus Enam Puluh Tujuh',
    );
  ...
  test('digit-by-digit style works without decimalStyle option', () => {
    expect(toWords.convert(1.5)).toBe('Satu Koma Lima');
    expect(toWords.convert(0.05)).toBe('Nol Koma Nol Lima');
  });
});
