...
import msSg from '../src/locales/ms-SG.js';
import {
  ToWords as LocaleToWords,
  toWords as localeToWords,
  toOrdinal as localeToOrdinal,
  toCurrency as localeToCurrency,
} from '../src/locales/ms-SG.js';

const localeCode = 'ms-SG';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toWords.getLocaleClass()).toBe(msSg);
  });
...
describe('Test Negative Integers with options = { currency: true }', () => {
  ...
  testNegativeIntegersWithCurrency.map((row, i) => {
    ...
    row[0] = -row[0];
    row[1] = `Minus ${row[1]} Dolar Sahaja`;
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `Seratus Dua Puluh Tiga Dolar Sahaja`],
    ['123', `Seratus Dua Puluh Tiga Dolar Sahaja`],
    ['123.0', `Seratus Dua Puluh Tiga Dolar Dan Sifar Sen Sahaja`],
    ['123.00', `Seratus Dua Puluh Tiga Dolar Dan Sifar Sen Sahaja`],
    ['0.00', `Sifar Dolar Dan Sifar Sen Sahaja`],
    ['-123.00', `Minus Seratus Dua Puluh Tiga Dolar Dan Sifar Sen Sahaja`],
    ['37.68', `Tiga Puluh Tujuh Dolar Dan Enam Puluh Lapan Sen Sahaja`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, 'Sifar Dolar Sahaja'],
  [0.04, 'Sifar Dolar Dan Empat Sen Sahaja'],
  [0.0468, 'Sifar Dolar Dan Lima Sen Sahaja'],
  [0.4, 'Sifar Dolar Dan Empat Puluh Sen Sahaja'],
  [0.63, 'Sifar Dolar Dan Enam Puluh Tiga Sen Sahaja'],
  [0.973, 'Sifar Dolar Dan Sembilan Puluh Tujuh Sen Sahaja'],
  [0.999, 'Satu Dolar Sahaja'],
  [37.06, 'Tiga Puluh Tujuh Dolar Dan Enam Sen Sahaja'],
  [37.068, 'Tiga Puluh Tujuh Dolar Dan Tujuh Sen Sahaja'],
  [37.68, 'Tiga Puluh Tujuh Dolar Dan Enam Puluh Lapan Sen Sahaja'],
  [37.683, 'Tiga Puluh Tujuh Dolar Dan Enam Puluh Lapan Sen Sahaja'],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace('Sifar Dolar Dan ', '');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('zero with currency', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Sifar Dolar Sahaja');
  });
});
...
const testFractionStyleMsSG: [number, string][] = [
...
describe("Test Floats with options = { decimalStyle: 'fraction' }", () => {
  test.concurrent.each(testFractionStyleMsSG)('convert %d => %s', (input, expected) => {
