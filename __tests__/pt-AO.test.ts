...
import ptAo from '../src/locales/pt-AO.js';
import {
  ToWords as LocaleToWords,
  toWords as localeToWords,
  toOrdinal as localeToOrdinal,
  toCurrency as localeToCurrency,
} from '../src/locales/pt-AO.js';

const localeCode = 'pt-AO';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toWords.getLocaleClass()).toBe(ptAo);
  });
...
describe('Test with options = { currency: true, includeZeroFractional: true }', () => {
  const testIncludeZeroFractional: [number | string, string][] = [
    [123, `Cento E Vinte E Três Kwanzas`],
    ['123', `Cento E Vinte E Três Kwanzas`],
    ['123.0', `Cento E Vinte E Três Kwanzas E Zero Cêntimos`],
    ['123.00', `Cento E Vinte E Três Kwanzas E Zero Cêntimos`],
    ['0.00', `Zero Kwanzas E Zero Cêntimos`],
    ['-123.00', `Menos Cento E Vinte E Três Kwanzas E Zero Cêntimos`],
    ['37.68', `Trinta E Sete Kwanzas E Sessenta E Oito Cêntimos`],
  ];
...
const testFloatsWithCurrency: [number, string][] = [
  [0.0, `Zero Kwanzas`],
  [0.04, `Zero Kwanzas E Quatro Cêntimos`],
  [0.0468, `Zero Kwanzas E Cinco Cêntimos`],
  [0.4, `Zero Kwanzas E Quarenta Cêntimos`],
  [0.63, `Zero Kwanzas E Sessenta E Três Cêntimos`],
  [0.973, `Zero Kwanzas E Noventa E Sete Cêntimos`],
  [0.999, `Um Kwanza`],
  [37.06, `Trinta E Sete Kwanzas E Seis Cêntimos`],
  [37.068, `Trinta E Sete Kwanzas E Sete Cêntimos`],
  [37.68, `Trinta E Sete Kwanzas E Sessenta E Oito Cêntimos`],
  [37.683, `Trinta E Sete Kwanzas E Sessenta E Oito Cêntimos`],
];
...
describe('Test Floats with options = { currency: true, ignoreZeroCurrency: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreZeroCurrency.map((row, i) => {
    ...
    if (row[0] > 0 && row[0] < 1) {
      row[1] = (row[1] as string).replace(`Zero Kwanzas E `, '');
    }
  });
...
describe('Test Floats with options = { currency: true, ignoreDecimal: true }', () => {
  ...
  testFloatsWithCurrencyAndIgnoreDecimal.map((row) => {
    if (row[0] === 0.999) {
      ...
      row[1] = (row[1] as string).replace(new RegExp(`Kwanzas.*`), 'Kwanzas');
    }
  });
...
describe('Test Zero Variants', () => {
  ...
  test('convert 0 with currency => Zero Kwanzas', () => {
    expect(toWords.convert(0, { currency: true })).toBe('Zero Kwanzas');
  });
});
